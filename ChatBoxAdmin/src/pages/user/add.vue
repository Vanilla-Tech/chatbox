...
export default {
  ...
  beforeRouteEnter(from, to, next) {
    next(p => {
      p.$store.dispatch("user/initUserAdd", {});
    });
  },
  computed: {
    customerImage: {
      get() {
        return this.$store.state.user.user.customerImage;
      },
      set(value) {
  ...
  data() {
    return {
      ...
      options: [
        { id: "5cac21f02a83bd49c820479d", text: "Department one" },
        { id: "5cac21f02a83bd49c82047a5", text: "Department two" }
      ],
      optionRoles: [{ id: 1, text: "Role one" }, { id: 2, text: "Role two" }]
    };
  },
  methods: {
