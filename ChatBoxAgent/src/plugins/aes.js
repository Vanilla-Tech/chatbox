...
var chatEncryptor=(function() {
    var key = CryptoJS.enc.Base64.parse("TWVyY2hhbnRyYWRlTW9uZXlDaGF0IUAjJF4pKComIUA=");
    function maskCard(inputText){
      var cardnormal =  /(?:\d{4}[ -/]?){3}(?=\d{4}|\d{3})/gm
      //var express =  /\b(?:\d{4}[ -]?){3}(?=\d{3}\b)/gm
      const subst = `**** **** **** `
      if (inputText.match(cardnormal)) {
       return inputText.replace(cardnormal, subst)
      }
      // if (inputText.match(express)) {
      //  return inputText.replace(express, subst)
      // }
      return inputText
    }
    ...
    //     var cardnomaster = /(?:5[0-5][0-9]{14})/gim;
    ...
    //     var maskregex = /.*(\d{4})/;
    //     return data.replace(maskregex, "************$1");
    //   }
...
export default chatEncryptor=chatEncryptor;